...
import { saveBlogCategoryAction, deleteBlogCategoryAction } from "@/app/admin/actions";
...
export default async function EditBlogCategoryPage({
  ...
  return (
    <AdminShell title={`Edit: ${category.name}`}>
      ...
      <form action={saveBlogCategoryAction} className="grid gap-6 max-w-2xl">
        ...
        <section className="rounded-sm border border-black/10 bg-white p-6">
          ...
          <div className="grid gap-5">
            ...
            <label className="grid gap-2 text-sm font-medium text-black">
              Slug
              <input name="slug" defaultValue={category.slug} required className="border border-black/15 px-3 py-3 font-mono text-sm outline-none focus:border-black/40" />
            </label>
            <label className="grid gap-2 text-sm font-medium text-black">
              Description
            ...
            <label className="grid gap-2 text-sm font-medium text-black">
              Sort order
              <input name="sortOrder" type="number" defaultValue={category.sortOrder} className="border border-black/15 px-3 py-3 outline-none focus:border-black/40" />
            </label>
      ...
      <form action={deleteBlogCategoryAction} className="mt-10 max-w-2xl border border-red-200 bg-red-50 p-6">
        <input type="hidden" name="id" value={category.id} />
        <h2 className="text-lg font-semibold text-red-800">Danger zone</h2>
        <p className="mt-1 text-sm text-red-700">Posts in this category will be uncategorized, not deleted.</p>
